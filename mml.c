...
int
mml_param_valid(struct mml_param *param)
{
  return (param->name && *param->name && param->value);
}

void
sort_mml_command(struct mml_command *mml, int params, struct mml_param param[])
{
  int p;
  ...
  for (p = 0; p < mml->params; p++) {
    ...
    if (!mml_param_valid(&mml->param[p]))
      continue;
...
void
free_mml_command(struct mml_command *mml)
{
  int p;
...
dequote(char *str)
{
  ...
  for (p1 = str, p2 = nstr; *p1; p1++, p2++)
    if (*p1 == '"') p2--, inside = !inside; else if (*p1 == ' ' && !inside) p2--; else *p2 = *p1;
  *p2 = 0;
